...
def predict_vm(ecs_lines, input_lines, input_test_file_array=None):
    ...
    vm_size, vm, pm_size, pm, res_use_pro, other_res_use_pro, pm_free = packing_processer.pack_all(caseInfo, predict_result)
...
def result_modify1(predict_result, caseInfo, try_value, vm_type):
    '''
    ...
    if try_res_use_pro > res_use_pro and try_pm_size <= pm_size:  # 如果结果优,物理机数量相等或者 【更小,利用率更高 】保存最优结果
        ...
        # 继续深度搜索
        result_modify1(try_predict, caseInfo, try_value, vm_type)
...
def result_smooth(vm_size, vm, pm_size, pm, caseInfo,pm_free):
            ...
            if pm_free[i][0] and pm_free[i][1]:
                ...
                while(M_C>=1 and pm_free[i][0] and pm_free[i][1]):#CPU 内存均有空间
                    ...
                    for vm_type_index in range(len(VM_PARAM) - 1, -1, -1):
                        ...
                        if VM_PARAM[VM_TYPE_DIRT[vm_type_index]][2]==M_C and (VM_TYPE_DIRT[vm_type_index] in vm_types):
                            # CPU 内存均有空间放入该虚拟机
                            if VM_PARAM[VM_TYPE_DIRT[vm_type_index]][0]<=pm_free[i][0] and VM_PARAM[VM_TYPE_DIRT[vm_type_index]][1]<=pm_free[i][1]:
                                #虚拟机数量增加
                                vm_size+=1
                                #列表中数量添加
    ...
    res_use_pro=1.0-res_use_pro
    other_res_use_pro=1.0-other_res_use_pro
